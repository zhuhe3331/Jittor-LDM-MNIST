...
from pytorch_version.sampler.DDIMSampler import DDIMSampler
from pytorch_version.utils import train_DDPM

# 加载配置文件
cfg = OmegaConf.load('./config/ddim_cond.yaml')
...
for epoch in range(epochs):
    ...
    avg_epoch_loss = train_DDPM(train_loader, optimizer, ddim, d_cond, device)
...
plt.legend()
plt.show()
plt.savefig(f"./losses/mnist_ddim_cond_{time.time()}.png")
plt.close()

with open('./results/loss_ddim_cond.txt', 'w') as f:
...
generated_images = ddim.sample((10, 1, image_size, image_size), cond)
...
plt.show()
plt.savefig(f"./results/mnist_ddim_cond_{time.time()}.png")
plt.close()
