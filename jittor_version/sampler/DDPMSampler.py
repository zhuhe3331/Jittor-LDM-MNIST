...
class DDPMSampler(DiffusionSampler):
    def __init__(self, model: LatentDiffusion):
        ...
        with jt.no_grad():
            ...
            alpha_bar_prev = jt.concat([jt.array([1.]), alpha_bar[:-1]])
            ...
            self.log_var = jt.log(jt.clamp(var, min_v=1e-20)).numpy()
    ...
    def p_sample(self, x, c, t, step,
        ...
        sqrt_recip_alpha_bar = jt.full((bs, 1, 1, 1), self.sqrt_recipe_alpha_bar[step])
        sqrt_recip_m1_alpha_bar = jt.full((bs, 1, 1, 1), self.sqrt_recipe_m1_alpha_bar[step])

        x0 = sqrt_recip_alpha_bar * x - sqrt_recip_m1_alpha_bar * eps_t

        mean_x0_coef = jt.full((bs, 1, 1, 1), self.mean_x0_coef[step])
        mean_xt_coef = jt.full((bs, 1, 1, 1), self.mean_xt_coef[step])

        mean = mean_x0_coef * x0 + mean_xt_coef * x

        log_var = jt.full((bs, 1, 1, 1), self.log_var[step])
    ...
    def q_sample(self, x0, index, noise=None):
        ...
        # print(self.sqrt_alpha_bar[index].shape)
        # print(self.sqrt_1m_alpha_bar[index].shape)
        # print(x0.shape)
        # print(noise.shape)
    ...
    def loss(self, x0, c, noise=None):
        ...
        size = jt.size(eps_theta)

        loss = nn.mse_loss(eps_theta, noise, reduction="mean")
        # loss = (eps_theta - noise).abs().sum()
        # loss = np.nanmean((eps_theta - noise) ** 2)
        # if math.isnan(loss):
        #     # print(eps_theta)
        #     # print(noise)
        #     print(np.sum(np.isnan(xt)))
        #     print(np.sum(np.isnan(t)))
        #     print(np.sum(np.isnan(c)))
        #     print(np.sum(np.isnan(noise)))
        #     print(np.sum(np.isnan((eps_theta - noise)**2)))
        return loss
...
def _test_q_sample():
    ...
    tf = transform.Compose([
        ...
        # transform.ConvertImageDtype(jt.float),
        transform.ImageNormalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
    ])
    img = jt.array(tf(image))
    ...
    for idx, t in enumerate([0, 50, 100, 200, 300, 499]):
        ...
        noisy_image = (x_noisy.squeeze(dim=0).permute(1, 2, 0) + 1) * 127.5
