...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, channels: int, n_heads: int, n_layers: int, d_cond: int):
        ...
        self.norm = jt.nn.GroupNorm(num_groups=32, num_channels=channels, eps=1e-6, affine=True)
    ...
    def execute(self, x: jt.Var, cond: jt.Var):
        """
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_head: int, d_cond: int):
        ...
        # Feed-execute network and pre-norm layer
...
class CrossAttention(nn.Module):
    ...
    def execute(self, x: jt.Var, cond: Optional[jt.Var] = None):
        """
        :param x: are the input embeddings of shape `[batch_size, height * width, d_model]`
        :param cond: is the conditional embeddings of shape `[batch_size, n_cond, d_cond]`
        """
    ...
    def normal_attention(self, q: jt.Var, k: jt.Var, v: jt.Var):
        """
        #### Normal Attention
        ...
        q = jt.view(q, (*q.shape[:2], self.n_heads, -1))
        k = jt.view(k, (*k.shape[:2], self.n_heads, -1))
        v = jt.view(v, (*v.shape[:2], self.n_heads, -1))

        # Calculate attention $\frac{Q K^\top}{\sqrt{d_{key}}}$
        attn = jt.linalg.einsum('bihd,bjhd->bhij', q, k) * self.scale
        ...
        out = jt.linalg.einsum('bhij,bjhd->bihd', attn, v)
